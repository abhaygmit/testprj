	...
	function createDataset()
	{
		...
		$totalApt=($examNeeded+$startNeeded+$treatmentNeeded+$finishNeeded+$recordNeeded+$otherNeeded);
		 ...
		 foreach($docTimeInPer as $docExam)
		 {
			...
			$docs[$docno]=array('Start'=>round((($startPerD*$docExam['time_percent'])/100)),
		...
		foreach($breaks as $break)
			{
				...
				$op_breaks[$break_type]=array('interval'=>$breakTime, 'time'=>$break['break_time'], 'SME_option'=>$break['SME_option']);	
			}
		...
		$response['docEstimatedApts']=array('no_of_est_apts'=>round($totalApt), 'no_of_docs'=>$no_of_docs, $docs);	
		$response['workingHours']=array($_POST['aptDy']=>$_POST['aptHours']);	
		...
		foreach($midProcs as $m){
			...
			foreach($sel as $key=>$value){
				$ddd[$key]=$value['assign_time'];
			}				
			$e=$m['proc_abbr'];
			$PM[$e]=array('proc_id'=>$m['id'], 'color_code'=>$m['colorCode'], 'length'=>$m['procedure_length_in_minutes'], 'weight'=>$m['proc_weight'], 'docAvailability'=>array($ddd));
		...
		foreach($startProcs as $m){
			...
			$e=$m['proc_abbr'];
			$PS[$e]=array('proc_id'=>$m['id'], 'color_code'=>$m['colorCode'], 'length'=>$m['procedure_length_in_minutes'], 'weight'=>$m['proc_weight'], 'docAvailability'=>array($ddd));
		...
		foreach($endProcs as $m){
			...
			$e=$m['proc_abbr'];
			$PE[$e]=array('proc_id'=>$m['id'], 'color_code'=>$m['colorCode'], 'length'=>$m['procedure_length_in_minutes'], 'weight'=>$m['proc_weight'], 'docAvailability'=>array($ddd));
