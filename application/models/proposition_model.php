...
class Proposition_model extends CI_Model 
{
	...
	function getPropList()
		{
			$query=$this->db->query("select * from  matches where status=1")->result();
			return $query;
		}
	...
	function add_edit_props($post, $id=null)
	{
		@$dt=date('Y-m-d H:i:s');
		...
		if($id!=''){
			...
			$data=array(
						...
						'proposition_title'=>$_POST['proposition_title'],
						'status'=>$_POST['status'],
						'updated_at'=>$dt
						
					);
			$this->db->where('id', $id);
			return $this->db->update('propositions', $data);
		...
		$data=array(
			...
			return $this->db->insert('propositions', $data);
	...
	{
		//$this->db->where('id', $id);
		return $this->db->query("delete from propositions where id='$id'");
	}
