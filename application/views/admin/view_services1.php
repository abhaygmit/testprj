...
function resetForm()
{
   ...
   window.location.href='<?php echo base_url()?>index.php/services/view_services';
}
</script>
	<?php 
	...
	<div class="rightColm">
        <div class="result_opt">
            <div class="left"><h2>Manage Services</h2></div>
        </div>
        <div class="clear"></div>
        <form action="<?php echo site_url('services/view_services')?>" id="generate" name="generate" method="post">
            <table cellspacing="3" width="90%" style="border:1px solid #666666;">
            <tr>
           		 <td height="30" colspan="3" style="font-size:14px;"><strong>Search Service/Category Name By</strong></td>
            </tr>
            <tr>
            <td><strong>Service Name:</strong></td>
            <td><input type="text" name="service_name" id="service_name" value="<?php echo ($_POST["service_name"]!=''? trim(htmlentities($_POST["service_name"])):"");?>" style="border:thin inset" /></td>
            ...
            <div class="tablWrap viewPerm">
			 	<table cellspacing="0" cellpadding="0" class="tHeader">
              		<tr>
                		...
                		<th class="col_pname1">Service Name</th>
                		<th class="col_pname1">Category Name</th>
                		<th class="col_pname1">Service Status</th>
	...
	<?php
			...
			if(count($categoryDetails)>0)
			{
				...
				?>  
                    <tr>
						...
						<td class="col_pname1"><?php if(sizeof($childs)>0){ ?><a href="javascript:void(null);" style="vertical-align:middle;text-decoration:none; " onclick="show_more(<?=($i+1)?>);imageChange('img<?php echo $i;?>');"><img src='images/admin/<?php if($i==1) echo 'ico_minus'; else echo 'ico_plus';?>.png' id="img<?php echo $i;?>"/>&nbsp;<?php echo ucwords($categoryDetail->service_name);?></a> <?php }else{ ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo ucwords($categoryDetail->service_name);?> <?php } ?> 
						</td>
						<td class="col_pname1">
							<?php echo $catArr[0]->category_name;?>
						</td>
						<td class="col_pname1">
							<?php echo ($categoryDetail->status=='0' ? 'In-active': 'Active');?>
						</td>
						<td class="col_cState" >
				   			<a href="<?php echo site_url('services/edit/'.$categoryDetail->id);?>" title="Edit">
                            	<img src="images/admin/edit.png"  />
                            </a>&nbsp;
                            <?php if(count($childs)==0){?>
                            	<a href="<?php echo site_url('services/deleteservice/'.$categoryDetail->id);?>" onclick="return confirm('Do you want to delete this service?')" title="Delete" alt="Delete">
                           <?php }else{?>
        ...
        <?php if(($_POST['service_name']=='') && ($_POST['cat_name']=='') && ($_POST['status']=='')){ ?>
        <div class="paging">
