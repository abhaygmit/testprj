 ...
 */
function codaserver_connect($hostname = 'localhost', $port = '3407', $username = 'root', $password = 'admin') {
 ...
 * @return resultset array
 */
function codaserver_show_groups($resource_identifier, $application_name = null, $environment = null, $where_clause = null, $order_by_clause = null) {
	if (!empty($application_name) && !in_array(strtolower($environment), array('dev', 'test', 'prod')) ) {
		throw new Exception('Application must have an environment specified');
	} else if (!empty($environment) && empty($application_name)) {
		throw new Exception('Environment requires an application be specified');
	}
	return codaserver_query($resource_identifier, 'SHOW GROUPS '.(!empty($application_name) ? ' FOR APPLICATION '.$application_name.'.'.$environment : '').(!empty($where_clause) ? ' WHERE '.$where_clause : '').(!empty($order_by_clause) ? ' ORDER BY '.$order_by_clause : ''));
}
